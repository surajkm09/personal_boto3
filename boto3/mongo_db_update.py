...
def insert_into_db(uuid ,url ,topics,voice):
    client=MongoClient()
    db=client.user_information
    find={}
    find['uuid']=uuid

    if db.information.find(find).count()>0:
        new={}
        new['uuid']=uuid
        new['url']=url
        new['Topics']=topics
        new['voice']=voice
        db.information.replace_one(find,new)
        ...
        new['Topics'] = topics
        new['voice'] = voice
        db.information.insert_one(new)
        arn=find_arn(topics)
        initial_subscribe(arn,url)
    result=db.information.find()
    for cursor in result:
        print(cursor)
def initial_subscribe(arn,http_endpoint):
    client = boto3.client('sns')
    for i in arn:
        response=client.subscribe(TopicArn=i,Protocol='http',Endpoint=http_endpoint)
        print(i)
        print(response)
